...
class _AnnAssignVisitor:
    ...
    def _Name(self, node):
        ...
        self._assigned(node.row_id, assignment)
...
class _ScopeVisitor(_ExpressionVisitor):
    ...
    def _FunctionDef(self, node):
        ...
        for decorator in pyfunction.decorators:
            if isinstance(decorator, ast.Name) and decorator.row_id == "property":
...
class _ComprehensionVisitor(_ScopeVisitor):
    ...
    def _Name(self, node):
        if isinstance(node.ctx, ast.Store):
            self.names[node.row_id] = self._get_pyobject(node)
...
class _ClassInitVisitor(_AssignVisitor):
    ...
    def _Attribute(self, node):
        if not isinstance(node.ctx, ast.Store):
            return
        if isinstance(node.value, ast.Name) and node.value.row_id == self.self_name:
