    ...
    def _get_all_star_list(self, pymodule):
        ...
        while assignments:
            ...
            if isinstance(assignment, ast.List):
                ...
                while stack:
                    ...
                    if isinstance(el, ast.IfExp):
                        ...
                        if isinstance(el, ast.Str):
                            ...
                            try:
                                name = pymodule.get_attribute(el.row_id)
                            except exceptions.AttributeNotFoundError:
                                continue
                            else:
                ...
                try:
                    name = pymodule.get_attribute(assignment.row_id)
                except exceptions.AttributeNotFoundError:
                    continue
                else:
                    assignments.extend(
...
class _UnboundNameFinder:
    ...
    def _Name(self, node):
        if self._get_root()._is_node_interesting(node) and not self.is_bound(node.row_id):
            self.add_unbound(node.row_id)
